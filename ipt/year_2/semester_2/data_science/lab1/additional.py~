...
main_path = '/home/nikita/Notebooks/SRP/lab1'
...
def makeFig(df): # Создать график
    ...
    hdr.plt.xticks( hdr.np.linspace(0, weeks_total-weeks_2016, 8), hdr.np.linspace(1981, 2016, 8).astype(int))
    hdr.plt.xlabel("Года")
    hdr.plt.ylabel("VHI")
    axes.plot(x,y)
    hdr.plt.show()

def makeDF(current_name): # Создать датафрейм
    print(current_name)
...
def makeExtVHI(df): # Отобразить максимальное/минимальное значение
    maxVHI = df.VHI.idxmax()
    minVHI = df.VHI.idxmin()
    print("Максимальное значение VHI:")
    hdr.display(df.loc[[maxVHI]])
    print("Минимальное значение VHI:")
    hdr.display(df.loc[[minVHI]])
